---
name: workking4
description: Start the full Nepal Instagram creator discovery workflow for Gandaki Province with /workking4.
user-invocable: true
---

# Workking4

`workking4` runs the full `workking` workflow locked to `Gandaki Province`.
...
- Instagram only
- Nepal only
- province scope must stay `Gandaki Province`
...
2. Search Instagram only within `Gandaki Province`. Do not search Nepal broadly and filter later.
...
8. Keep the run state under `runtime/state.json` consistent with:
   - `trigger_command = "/workking4"`
   - `last_active_province = "Gandaki Province"`
