---
name: workking7
description: Start the full Nepal Instagram creator discovery workflow for Sudurpashchim Province with /workking7.
user-invocable: true
---

# Workking7

`workking7` runs the full `workking` workflow locked to `Sudurpashchim Province`.
...
```powershell
python {baseDir}/../workking/scripts/workking_runner.py start --province "Sudurpashchim Province" --trigger-command "/workking7"
```
...
- Instagram only
- Nepal only
- province scope must stay `Sudurpashchim Province`
...
2. Search Instagram only within `Sudurpashchim Province`. Do not search Nepal broadly and filter later.
...
8. Keep the run state under `runtime/state.json` consistent with:
   - `trigger_command = "/workking7"`
   - `last_active_province = "Sudurpashchim Province"`
