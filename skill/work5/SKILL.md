---
name: work5
description: Start the full Nepal Instagram creator discovery workflow for Lumbini Province with /work5.
user-invocable: true
---

# Work5

`work5` runs the full `workking` workflow locked to `Lumbini Province`.
...
```powershell
python {baseDir}/../workking/scripts/workking_runner.py start --province "Lumbini Province" --trigger-command "/work5"
```
...
- Instagram only
- Nepal only
- province scope must stay `Lumbini Province`
...
2. Search Instagram only within `Lumbini Province`. Do not search Nepal broadly and filter later.
...
8. Keep the run state under `runtime/state.json` consistent with:
   - `trigger_command = "/work5"`
   - `last_active_province = "Lumbini Province"`
